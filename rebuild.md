...
1. **`web/public/locales/en/config/{global,cameras}.json` were hand-edited** for
   ...
   sensible.
2. Tests were never executed. The suite cannot run on native Windows
   (`frigate/log.py` calls `os.register_at_fork`, POSIX-only; plus cv2/ruamel/
   peewee/onnxruntime). Verified only by byte-compile + a stubbed-import harness.
   **Run `pytest frigate/test/test_parkpow.py frigate/test/test_config.py -k lpr`
   in the container.**
...
| Alarm UI visual QA | Compiles and type-checks; never opened in a browser |
