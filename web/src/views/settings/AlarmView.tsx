...
const STATE_BADGE_CLASSES: Record<AlarmState, string> = {
  ...
  armed_stay: "bg-blue-600 text-white",
...
export default function AlarmView() {
  ...
  if (!status || !status.state) {
    return (
      <div className="p-4 text-sm text-muted-foreground">{t("notEnabled")}</div>
    );
  }

  const canDisarm =
    status.state !== "disarmed" && status.state !== "alarm_memory";
  const canClear = status.state === "alarm_memory";
  ...
  return (
    <div className="scrollbar-container flex size-full flex-col overflow-y-auto p-2">
      ...
      <div className="flex flex-wrap items-center gap-3">
        ...
        {status.armed_mode && (
          <Badge variant="outline">{t(`armedMode.${status.armed_mode}`)}</Badge>
        )}
        ...
        <div className="flex flex-wrap gap-2">
          {status.zones.map((zone) => (
            <Badge
              ...
              variant={
                !zone.enabled ? "outline" : zone.armed ? "secondary" : "outline"
              }
            >
